...
const CreateEvent: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const eventDate = new Date(form.startDateTime)
    const now = new Date()
    if (eventDate <= now) {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.error || error.response?.data?.details || "Failed to create event"
    ...
    <div className="bg-gradient-to-br from-white to-gray-50 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200/50 p-6 sm:p-8">
      ...
      {!user?.isCalendarConnected ? (
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl shadow-lg border border-amber-200/50 p-8 sm:p-12 text-center">
          ...
          <p className="text-gray-600 text-lg">Please connect your Google Calendar first to create events.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="space-y-6">
            ...
            <div className="group">
              <label htmlFor="startDateTime" className="block text-sm font-semibold text-gray-800 mb-3 group-focus-within:text-blue-600 transition-colors">
                Date & Start Time *
              </label>
              <div className="relative">
                <input
            ...
            <div className="group">
              <label htmlFor="duration" className="block text-sm font-semibold text-gray-800 mb-3 group-focus-within:text-blue-600 transition-colors">
                Duration
              </label>
              <div className="relative">
                <select
