...
router.get("/google/callback", async (req, res) => {
  try {
    ...
    if (error) {
      ...
      const errorMessages = {
        access_denied:
          "Access was denied. Please try again or contact the developer.",
        unauthorized_client:
          "App is not authorized. Please contact the developer.",
    ...
    if (!code) {
      return res.redirect(
        `${process.env.FRONTEND_URL}?auth=error&message=no_code`
      );
    }
    ...
    if (user) {
      ...
      const username = userInfo.email?.split("@")[0] || `user_${userInfo.id}`;
    ...
    req.session.save((err) => {
      if (err) {
        ...
        return res.redirect(
          `${process.env.FRONTEND_URL}?auth=error&message=session_error`
        );
      }
